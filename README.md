...
## DAMSAL arbitration layer

This bundle includes the **Degradation-Aware Multi-Strategy Arbitration Layer (DAMSAL)**. DAMSAL compares S0-S6 control candidates using clean and degraded solver outputs and determines whether S3 is actually superior.

Run:

```bash
python run_arbitration_layer.py --clean_csv clean_baseline_daily.csv --degraded_csv degraded_scenario_daily.csv --out_dir arbitration_outputs
```

or use the standalone UI:
